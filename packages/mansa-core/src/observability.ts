...
export interface MetricPoint { name: string; kind: MetricKind; value: number; timestamp: Date; labels: Readonly<Record<string, string>>; }
export interface TraceSpan { traceId: string; spanId: string; name: string; startedAt: Date; endedAt?: Date; durationMs?: number; status: 'OK' | 'ERROR'; attributes: Readonly<Record<string, string>>; }

export class ObservabilityRegistry {
  private readonly metrics: MetricPoint[] = [];
  private readonly spans = new Map<string, TraceSpan>();

  recordMetric(input: Omit<MetricPoint, 'timestamp' | 'labels'> & { timestamp?: Date | undefined; labels?: Record<string, string> | undefined }): MetricPoint {
    ...
    const point: MetricPoint = Object.freeze({ name: input.name, kind: input.kind, value: input.value, timestamp: input.timestamp ?? new Date(), labels: Object.freeze({ ...(input.labels ?? {}) }) });
  ...
  startSpan(input: { traceId: string; spanId: string; name: string; startedAt?: Date | undefined; attributes?: Record<string, string> | undefined }): TraceSpan {
    if (this.spans.has(input.spanId)) throw new Error(`span ${input.spanId} already exists`);
    const span: TraceSpan = Object.freeze({ traceId: input.traceId, spanId: input.spanId, name: input.name, startedAt: input.startedAt ?? new Date(), status: 'OK', attributes: Object.freeze({ ...(input.attributes ?? {}) }) });
    this.spans.set(input.spanId, span);
    return span;
  }

  endSpan(spanId: string, input?: { endedAt?: Date | undefined; status?: 'OK' | 'ERROR' | undefined; attributes?: Record<string, string> | undefined }): TraceSpan {
    ...
    const span: TraceSpan = Object.freeze({ ...existing, endedAt, durationMs, status: input?.status ?? existing.status, attributes: Object.freeze({ ...existing.attributes, ...(input?.attributes ?? {}) }) });
...
export interface AlertRule { id: string; metricName: string; operator: AlertOperator; threshold: number; severity: IncidentSeverity; cooldownMs: number; enabled: boolean; requiredLabels?: Readonly<Record<string, string>> | undefined; }
export interface IncidentAlert { id: string; ruleId: string; metricName: string; observedValue: number; threshold: number; severity: IncidentSeverity; openedAt: Date; status: 'OPEN' | 'ACKNOWLEDGED' | 'RESOLVED'; labels: Readonly<Record<string, string>>; }

function matches(operator: AlertOperator, value: number, threshold: number): boolean {
  switch (operator) { case 'GT': return value > threshold; case 'GTE': return value >= threshold; case 'LT': return value < threshold; case 'LTE': return value <= threshold; case 'EQ': return value === threshold; }
}
...
export class IncidentAlertManager {
  ...
  evaluate(metric: MetricPoint, now: Date = metric.timestamp): readonly IncidentAlert[] {
    ...
    for (const rule of this.rules.values()) {
      ...
      const alert: IncidentAlert = Object.freeze({ id: `incident-${++this.sequence}`, ruleId: rule.id, metricName: metric.name, observedValue: metric.value, threshold: rule.threshold, severity: rule.severity, openedAt: now, status: 'OPEN', labels: Object.freeze({ ...metric.labels }) });
  ...
  acknowledge(alertId: string): IncidentAlert { return this.transition(alertId, 'ACKNOWLEDGED'); }
  resolve(alertId: string): IncidentAlert { return this.transition(alertId, 'RESOLVED'); }
  list(status?: IncidentAlert['status']): readonly IncidentAlert[] { return [...this.alerts.values()].filter((alert) => !status || alert.status === status).map((alert) => ({ ...alert, labels: { ...alert.labels } })); }
